...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    @foreach($features as $feature)
        <div class="feature-card group hover:border-red-200 border-2 border-transparent bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            <!-- Icon Container with Gradient -->
            <div class="mb-6 relative">
                <div class="{{ $feature['color'] }} w-20 h-20 rounded-2xl flex items-center justify-center text-4xl shadow-lg group-hover:shadow-xl group-hover:scale-125 transition-all duration-400 transform">
                    {{ $feature['icon'] }}
                </div>
                <!-- Enhanced Gradient Overlay -->
                <div class="absolute inset-0 bg-gradient-to-br from-transparent to-red-600 rounded-2xl opacity-0 group-hover:opacity-25 transition-opacity duration-400"></div>
                <!-- Floating decorative element -->
                <div class="absolute top-2 right-2 w-3 h-3 bg-red-300 rounded-full opacity-0 group-hover:opacity-60 transition-opacity duration-300"></div>
            </div>

            <!-- Title -->
            <h3 class="text-xl font-bold text-gray-900 mb-3 group-hover:text-red-600 transition-colors duration-400">
                {{ __($feature['title']) }}
            </h3>

            <!-- Description -->
            <p class="text-gray-600 leading-relaxed mb-4 group-hover:text-gray-700 transition-colors duration-400">
                {{ __($feature['description']) }}
            </p>

            <!-- Arrow Icon -->
            <div class="flex items-center gap-2 text-red-600 opacity-0 group-hover:opacity-100 translate-x-0 group-hover:translate-x-1 transition-all duration-400">
