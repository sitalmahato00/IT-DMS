...
<!-- Student Header -->
<header class="bg-blue-600 dark:bg-blue-800 shadow-sm border-b border-blue-600 text-white">
        ...
        <div class="flex items-center justify-between gap-4 h-full">
            <div class="flex-1 min-w-0 flex items-center h-full">
                <div class="space-y-0 flex flex-col justify-center h-full">
                    ...
                    <p class="text-xs text-blue-100 line-clamp-1 leading-tight">{{ __('Student Dashboard - IT Department') }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3 flex-shrink-0 h-full">
                <!-- Dark Mode Toggle -->
                <button id="darkModeToggle" class="p-2 bg-blue-500 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 rounded-lg transition" title="{{ __('Toggle Dark Mode') }}">
                ...
                @endphp
                <div class="relative">
                    <button id="notifToggle" class="relative bg-blue-500 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 p-2 rounded-lg transition" aria-expanded="false">
                    ...
                    <div id="notifDropdown" class="hidden absolute right-0 mt-2 w-96 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                        ...
                        <div class="max-h-64 overflow-y-auto">
                            @forelse($__notifList as $n)
                                ...
                                @endphp
                                <div class="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-start gap-3 cursor-pointer {{ isset($n->read_at) && $n->read_at ? '' : 'bg-blue-50 dark:bg-blue-900/30' }}">
                                    <div class="flex-1">
                ...
                <div class="relative group">
                    <button class="flex items-center gap-2 bg-blue-500 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 px-3 py-2 rounded-lg transition">
                        @php
                            $user = auth()->user();
                        ...
                        @if($profilePhotoPath && Storage::disk('public')->exists($profilePhotoPath))
                            ...
                            <div class="w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center text-xs font-bold" style="display:none;">
                                {{ substr($user->name ?? 'S', 0, 1) }}
                            </div>
                        @else
                            <i class="bi bi-person-circle text-white text-lg"></i>
                        @endif
                        <span class="text-sm font-medium text-white max-w-[150px] truncate">{{ auth()->user()?->name ?? 'Student' }}</span>
                    </button>
                    <div class="hidden group-hover:block absolute right-0 mt-0 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-40">
                        <a href="{{ route('student.profile.edit') }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-700">
