                ...
                <div class="col-span-1 lg:col-span-2">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Full name <span class="text-blue-500 text-base">*</span></label>
                            <input name="name" required value="{{ old('name', $student->name) }}" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm focus:ring-1 focus:ring-red-500" />
                        </div>
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Email <span class="text-blue-500 text-base">*</span></label>
                            <input type="email" name="email" required value="{{ old('email', $student->email) }}" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm" />
                        </div>

                        <!-- Row 2: Phone | Department -->
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Phone <span class="text-blue-500 text-base">*</span></label>
                            <input type="tel" name="phone" required value="{{ old('phone', $student->phone) }}" maxlength="10" pattern="[0-9]{10}" inputmode="numeric" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,10)" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm" />
                        </div>
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Department <span class="text-blue-500 text-base">*</span></label>
                            <input name="department" required value="{{ old('department', $student->department ?? ($student->student->department ?? '')) }}" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm" />
                        </div>

                        <!-- Row 3: Semester | Status -->
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Semester <span class="text-blue-500 text-base">*</span></label>
                            <select name="semester" required class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm">
                        ...
                        <!-- Row 4: Gender | Batch Year -->
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Gender</label>
                            <select name="gender" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm">
                        ...
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Batch Year <span class="text-blue-500 text-base">*</span></label>
                            <select name="batch_year" required class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm">
                                <option value="">Select</option>
                        ...
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Date of birth (AD) <span class="text-blue-500 text-base">*</span></label>
                            <input type="date" name="date_of_birth" id="studentDateAd" required value="{{ old('date_of_birth', $student->student->date_of_birth ?? '') }}" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm focus:ring-1 focus:ring-red-500" />
                        </div>
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Date of birth (BS)</label>
                            <input type="text" name="date_of_birth_bs" id="studentDateBs" placeholder="YYYY-MM-DD" value="{{ old('date_of_birth_bs', $student->student->date_of_birth_bs ?? '') }}" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm" />
                        </div>

                        <!-- Row 6: Roll No -->
                        <div class="flex flex-col">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Roll No <span class="text-blue-500 text-base">*</span></label>
                            <input name="student_id" required value="{{ old('student_id', $student->student->roll_no ?? '') }}" class="flex-1 block w-full px-3 py-2 border rounded-md text-sm shadow-sm" />
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        <button type="button" onclick="history.back()" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
                        <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md text-sm font-medium hover:bg-red-700">Save changes</button>
                    </div>
                </div>
            </div>
        </form>
...
<script>
    ...
    // Auto-convert AD to BS
    const studentDateAd = document.getElementById('studentDateAd');
    const studentDateBs = document.getElementById('studentDateBs');
    
    if(studentDateAd) {
        // Real-time calculation on input event (as user types)
        studentDateAd.addEventListener('input', function() {
            if(this.value && this.value.length === 10) { // YYYY-MM-DD format
                convertAdToBs(this.value).then(v => { studentDateBs.value = v || ''; });
            }
        });
        // Also listen for change event as fallback
        studentDateAd.addEventListener('change', function() {
            if(this.value) {
                convertAdToBs(this.value).then(v => { studentDateBs.value = v || ''; });
            }
        });
    ...
    if(studentDateBs) {
        // Real-time calculation on input event (as user types)
        studentDateBs.addEventListener('input', function() {
            if(this.value && this.value.length === 10) { // YYYY-MM-DD format
                convertBsToAd(this.value).then(v => { studentDateAd.value = v || ''; });
            }
        });
        // Also listen for change event as fallback
        studentDateBs.addEventListener('change', function() {
            if(this.value) {
                convertBsToAd(this.value).then(v => { studentDateAd.value = v || ''; });
            }
        });
