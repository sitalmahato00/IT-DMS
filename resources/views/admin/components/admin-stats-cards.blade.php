{{--
    ...
    - Subtle red accents or icons
    
    Usage:
    @include('admin.components.admin-stats-cards', [
        'cards' => [
            ['title' => 'Total Students', 'value' => $totalStudents, 'icon' => 'bi-people', 'color' => 'red'],
            ['title' => 'Active Students', 'value' => $activeStudents, 'icon' => 'bi-person-check', 'color' => 'green'],
            ['title' => 'Alumni', 'value' => $alumniCount, 'icon' => 'bi-mortarboard', 'color' => 'blue'],
            ['title' => 'New Admissions', 'value' => $newAdmissions, 'icon' => 'bi-person-plus', 'color' => 'purple'],
        ]
    ])
--}}
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-5 hover:shadow-md transition-shadow duration-200">
            <div class="flex items-start justify-between">
                ...
                <div class="flex-shrink-0">
                    <div class="w-12 h-12 rounded-xl {{ $iconBgClasses }} flex items-center justify-center">
                        <i class="bi {{ $icon }} text-xl {{ $colorClasses }}"></i>
                    </div>
                </div>
            </div>
            @if(!empty($card['trend']))
            <div class="mt-3 flex items-center gap-1 text-sm">
                @if($card['trend'] > 0)
                    ...
                    <span class="text-green-600 dark:text-green-400 font-medium">+{{ $card['trend'] }}%</span>
                    <span class="text-gray-500 dark:text-gray-400">vs last month</span>
                @elseif($card['trend'] < 0)
                    <i class="bi bi-arrow-down text-red-500"></i>
                    <span class="text-red-600 dark:text-red-400 font-medium">{{ $card['trend'] }}%</span>
                    <span class="text-gray-500 dark:text-gray-400">vs last month</span>
                @endif
            </div>
            @endif
