                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-slate-700" id="electivesTableBody">
                        @foreach($electiveSubjects as $subject)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition elective-row" data-semester="{{ $subject->semester }}">
                                <td class="px-6 py-3">
                                ...
                                <td class="px-6 py-3">
                                    @if($subject->subject_type === 'elective')
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-800">
                                            <i class="bi bi-shuffle"></i> {{ __('Elective') }}
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800">
                                ...
                                <td class="px-6 py-3 text-center">
                                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $subject->max_students && $subject->approved_count >= $subject->max_students ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' }}">
                                        {{ $subject->approved_count }}
                                    </span>
                                </td>
                                <td class="px-6 py-3 text-center">
                                    @if($subject->pending_count > 0)
                                        <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
                                ...
                                <td class="px-6 py-3 text-center">
                                    <button onclick="toggleEnrollment({{ $subject->id }}, {{ $subject->is_elective_open ? 'true' : 'false' }})"
                                        class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition {{ $subject->is_elective_open ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 hover:bg-green-200 dark:hover:bg-green-900/50' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-600' }}">
                                ...
                                <td class="px-6 py-3">
                                    <button onclick="assignStudent({{ $subject->id }}, '{{ $subject->subject_name }}', {{ $subject->semester }}, {{ $subject->max_students ?? 'null' }}, {{ $subject->approved_count }})"
                                        class="inline-flex items-center gap-1 px-3 py-1.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/50 rounded-lg text-xs font-medium transition">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                        @foreach($pendingEnrollments as $enrollment)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition" id="enrollment-row-{{ $enrollment->id }}">
                                <td class="px-6 py-3">
                                ...
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        <button onclick="approveEnrollment({{ $enrollment->id }})"
                                            class="inline-flex items-center gap-1 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-medium transition">
                                            <i class="bi bi-check2"></i> {{ __('Approve') }}
                                        </button>
                                        <button onclick="rejectEnrollment({{ $enrollment->id }})"
                                            class="inline-flex items-center gap-1 px-3 py-1.5 bg-red-100 text-red-700 hover:bg-red-200 rounded-lg text-xs font-medium transition">
...
<div id="assignModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        <div class="px-6 py-4 border-b bg-gray-50 flex items-center justify-between">
            <h3 class="text-base font-semibold text-gray-900">{{ __('Assign Student to Elective') }}</h3>
            <button onclick="closeAssignModal()" class="p-1.5 hover:bg-gray-200 rounded-lg transition">
        ...
        <div class="px-6 py-4 border-t bg-gray-50 flex justify-end gap-3">
            <button onclick="closeAssignModal()" class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg hover:bg-gray-100 text-gray-700 transition">
                {{ __('Cancel') }}
            </button>
            <button onclick="confirmAssign()" class="px-5 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition">
