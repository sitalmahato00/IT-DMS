...
<header class="bg-red-600 dark:bg-red-800 shadow-sm border-b border-red-500 text-white" data-mobile-app-header>
    <div class="px-6 py-0 h-16">
        <div class="flex items-center justify-between gap-4 h-full">
            <div class="flex min-w-0 flex-1 items-center gap-3">
                <button id="sidebarToggle" class="lg:hidden flex items-center justify-center w-9 h-9 rounded-lg bg-white text-red-700 border border-white/80 shadow-sm hover:bg-gray-100 transition flex-shrink-0" title="{{ __('Toggle Sidebar') }}">
                    <i class="bi bi-list text-lg"></i>
                </button>

                <div class="min-w-0">
                    <h2 class="text-lg font-bold text-white truncate leading-tight">{{ $resolvedTitle }}</h2>
                    <p class="text-xs text-red-100 truncate">{{ $resolvedSubtitle }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 flex-shrink-0 h-full">
                <div class="hidden md:block">
                    ...
                    <select id="locale-select" class="px-3 py-1.5 min-w-[120px] border border-gray-200 rounded-full text-sm text-gray-900 bg-white cursor-pointer focus:ring-2 focus:ring-red-500 focus:border-transparent shadow-sm transition dark:bg-slate-900 dark:text-white dark:border-slate-700">
                    ...
                    <div id="notifDropdown" class="hidden absolute right-0 mt-2 w-96 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-red-200 dark:border-slate-700 py-0 z-50 overflow-hidden">
                        <div class="px-4 py-3 border-b border-red-100 dark:border-slate-700 bg-red-50 dark:bg-slate-900/80 flex items-center justify-between">
                            <p class="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                                <i class="bi bi-bell text-red-700 dark:text-red-300"></i>
                                {{ __('Notifications') }}
                            </p>
                        ...
                        <div class="max-h-72 overflow-y-auto divide-y divide-gray-50 dark:divide-slate-700">
                            @forelse($__notifList as $notification)
                                ...
                                @endphp
                                <div class="px-4 py-3 hover:bg-red-50 dark:hover:bg-slate-700/60 flex items-start gap-3 transition {{ empty($notification->read_at) ? 'bg-red-50/70 dark:bg-red-950/20' : '' }}">
                ...
                <div class="relative">
                    <button id="profileToggle" class="flex items-center gap-2 p-1.5 border border-gray-200 bg-white text-gray-900 rounded-lg transition shadow-sm hover:bg-gray-50 dark:border-slate-700 dark:bg-slate-900/80 dark:text-white dark:hover:bg-slate-800">
                        ...
                        @endif
                        <span class="hidden md:block text-sm font-medium text-gray-900 dark:text-white max-w-[110px] truncate">{{ $user->name ?? __('Parent') }}</span>
                    ...
                    <div id="profileDropdown" class="hidden absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-red-200 dark:border-slate-700 py-1 z-50 overflow-hidden">
                        <div class="px-4 py-3 border-b border-red-100 dark:border-slate-700 bg-red-50 dark:bg-slate-900/80">
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $user->name ?? __('Parent') }}</p>
                            <p class="text-xs text-gray-500 dark:text-slate-400 truncate">{{ $user->email ?? '' }}</p>
                            <span class="inline-block mt-1 px-2 py-0.5 text-xs bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded-full font-medium">{{ __('Parent') }}</span>
                        </div>

                        <a href="{{ route('parent.profile.edit') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-900 dark:text-slate-100 hover:bg-red-50 dark:hover:bg-slate-700 transition">
                            <i class="bi bi-person-gear text-gray-400 dark:text-slate-400"></i>
                            {{ __('Profile Settings') }}
                        </a>

                        <a href="{{ route('parent.communication') }}#help" class="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-900 dark:text-slate-100 hover:bg-red-50 dark:hover:bg-slate-700 transition">
                            <i class="bi bi-life-preserver text-gray-400 dark:text-slate-400"></i>
                            {{ __('Help & Support') }}
                        </a>
